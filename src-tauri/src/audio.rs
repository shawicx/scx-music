...
fn try_output_stream_for_device(
    ...
    // Fallback: enumerate supported configs and try each one
    ...
    match device.supported_output_configs() {
        Ok(configs) => {
            ...
            // No supported configs worked or list was empty — try hardcoded
...
/// This bypasses both default_output_config() and supported_output_configs()
/// which can fail on certain CoreAudio devices.
fn try_build_hardcoded_stream(
    device: &rodio::cpal::Device,
) -> Result<(OutputStream, OutputStreamHandle), rodio::StreamError> {
...
fn find_device_by_name(name: &str) -> Result<rodio::cpal::Device, String> {
    ...
    // Prefer default output device handle if name matches.
    // This avoids picking the input-device variant of composite devices
    // (e.g., a speaker that also exposes a microphone input).
