...
describe('mirrorRenderer', () => {
  it('调用 clearRect 清空画布', () => {
    ...
    mirrorRenderer(ctx)
    expect(clearedArgs).toEqual([0, 0, 200, 100])
  })

  it('glow 模式不抛错', () => {
    expect(() => mirrorRenderer(makeCtx({ mode: 'glow' }))).not.toThrow()
  })

  it('flat 模式不抛错', () => {
    expect(() => mirrorRenderer(makeCtx({ mode: 'flat' }))).not.toThrow()
  })

  it('零频谱数据不抛错', () => {
    expect(() => mirrorRenderer(makeCtx({ frequencyData: new Uint8Array(64).fill(0) }))).not.toThrow()
  })
})
